...
enum BroadcastRadio {
    ...
    /// Step to the next or previous station WITHIN a kind, wrapping at both
    /// ends — the back and forward buttons never dead-end, and they never
    /// leave the kind the driver chose.
    static func step(from current: String?, in stations: [Station],
                     forward: Bool) -> Station? {
        guard !stations.isEmpty else { return nil }
        guard let current,
              let i = stations.firstIndex(where: { $0.id == current }) else {
            return forward ? stations.first : stations.last
        }
        let n = stations.count
        return stations[((i + (forward ? 1 : -1)) % n + n) % n]
    }
}
