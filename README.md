...
**Tested:** 163 Swift XCTest + 101 Rust tests, zero compiler warnings; `cargo clippy -D warnings` runs clean locally and is enforced by CI (`.github/workflows/ci.yml`, toolchain pinned in the workflow). The risk/scoring paths are pinned byte-identical to their original reference oracle's frozen fixtures; the RAPTOR engine is gated against a Dijkstra reference.
