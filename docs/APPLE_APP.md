...
apple/
  ...
  FLOWS/Sources/
    ...
    Resources/                # Assets.xcassets (FLOWS icon), app_risk_bundle.json
                              # (WI R-engine ZIP slice), nwr_stations.json,
...
| Per-ZIP family scores (all 11 families) | **National coverage** (2026-07): `app_risk_bundle.json` carries 33,300 ZCTAs — the 861 R-engine WI entries byte-preserved, plus 32,439 seasonal-climatology entries generated by `rust/flows-train/src/bin/national-bundle.rs` via `scripts/generate_national_bundle.sh` (weekly re-merge, R rows never touched). No polygon rings ship in the bundle — `ZCTAFetcher` (`Core/ZipBordersAndTransit.swift`) fetches ZCTA borders on demand |
...
| Alert severity map (Extreme .95 / Severe .88 / Moderate .72 / Minor .45) | **App approximation** (the R engine weights alerts per-family via its feeds) |
...
wind filtering, and the environmental blend work CONUS-wide, with the richer
WI engine export taking precedence where it exists (max of the two).
...
- **Weather visibility**: alert-polygon cap 12 → 40, corridor hazard shapes
  ...
  TAPPABLE → a summary card (hazard type, band, the R engine's ZIP hazard
  text, local score).
...
- **NA-wide viewport weather**: the live-conditions sweep runs at any zoom
  ...
  appear at continent scale wherever NWS grid conditions are elevated (the
  WI-only choropleth remains the R-engine export layer; live weather is no
  longer Wisconsin-bounded on screen). Verified in the iOS Simulator.
...
- **Stack rule**: the product is **Rust + AArch64 assembly + Swift only** —
...
- **Tests**: 153 Swift test functions (`apple/FLOWSTests/`) + 39 Rust
  (flows-core 37, flows-train 2).
...
4. Corridor weather: **done via the risk-bundle export, now national** —
   `scripts/export_app_risk_bundle.R` dumps the R engine's ZIP field
   (11 families + hazard summaries) from the warmed snapshot, and
   `scripts/generate_national_bundle.sh` merges the 32,439-ZCTA seasonal
   climatology tail around it (861 R rows byte-preserved →
   `data/runtime_cache/app_risk_bundle.json`, 33,300 zips);
   ...
   automate re-export on snapshot rewarm, and serve the bundle over HTTPS
   so iOS devices get it (macOS reads the repo file directly).
