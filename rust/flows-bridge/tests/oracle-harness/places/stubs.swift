...
import MapKit
import os
// Names the place files mention that the oracle never exercises; filled in as the compiler asks.
...
enum TripNeeds { enum Need { case fuel, food, rest } }
struct CorridorPartitionStub { var samples: [CLLocationCoordinate2D] = [] }
extension RouteService {
    nonisolated static func corridorPartition(of polyline: MKPolyline, everyMeters: Double) -> CorridorPartitionStub { CorridorPartitionStub() }
}
final class SeasonalRiskModel {
    static let shared = SeasonalRiskModel()
    func learnedHomeCoordinate(current: CLLocationCoordinate2D?) -> CLLocationCoordinate2D? { nil }
}
